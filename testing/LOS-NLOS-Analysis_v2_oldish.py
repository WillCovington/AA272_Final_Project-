...
def global_shadow_transition_times(sat_positions, times, L, X, AZ_b=0):
    """
    Returns a sorted list of timestamps at which ANY satellite
    enters or exits the building shadow region.

    These are the global NLOS ↔ LOS transition events.
    """
    theta_block = building_shadow_angle(L, X)
    all_events = []

    for sdat in sat_positions.values():
        az = np.mod(sdat["az"], 360)
        el = sdat["el"]

        # visible & NOT in shadow
        los = (el > 0) & ((az < AZ_b) | (az > AZ_b + theta_block))

        # visible & inside shadow
        nlos = (el > 0) & (az >= AZ_b) & (az <= AZ_b + theta_block)

        # 1 = LOS, -1 = shadow, 0 = below horizon
        state = np.where(los, 1, np.where(nlos, -1, 0))

        # detect transitions between +/-1 (ignore 0)
        for k in range(len(times) - 1):
            if state[k] != state[k+1] and 0 not in (state[k], state[k+1]):
                all_events.append(times[k+1])

    return np.array(sorted(all_events))
...
def global_mean_event_gap(sat_positions, times, L, X, AZ_b=0):
    """
    Mean time between ANY LOS↔NLOS transitions across ALL satellites.
    """
    events = global_shadow_transition_times(sat_positions, times, L, X, AZ_b)
...
def mean_gap_all_sats(sat_positions, times, L, X, AZ_b=0):
    """
    Wrapper so that the rest of your code continues working.
    """
    theta_block = building_shadow_angle(L, X)
    T_avg = global_mean_event_gap(sat_positions, times, L, X, AZ_b)
    return T_avg, theta_block
...
def plot_los_nlos_timeseries(sat_positions, times, L, X, AZ_b=0):
    ...
    ax.grid(True)
    ax.set_title(f"LOS (green) vs NLOS (red)\nBuilding at AZ={AZ_b}°, shadow={theta_block:.1f}°")
